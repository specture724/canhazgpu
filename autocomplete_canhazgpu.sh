_canhazgpu_complete() {
    ...
    case "$prev" in
        ...
        schedule)
            ...
            ;;
        guard)
            COMPREPLY=( $(compgen -W "--enforce --dry-run --once --interval --grace --confirmations --warn-interval --max-warnings --kill-grace --max-kills-per-hour --max-tasks-per-user --task-limit-hours --channels --log-file --exclude-users --exclude-commands --min-memory --notify-holder --no-maintenance --help" -- "$cur") )
            ;;
        violations)
